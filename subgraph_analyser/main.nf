...
include {MINIMAP_REDUNDANCY_REMOVER; BWAMEM2; BAM_FILTER; PATH_READ_ALN_FILTER} from "${projectDir}/path_filtering/path_filtering.nf"
...
workflow {
    ...
    if (params.kraken_db == "specify_path") {
        error "kraken2 database path must be specified in nextflow.config, expect large disk space requirements for standard refseq bacteria db"
    }
    if (params.genes == "specify_path") {
        error "gene database path must be specified in nextflow.config, default is CARD protein homolog fasta"
    }
    if (params.bakta_db == "specify_path") {
        error "bakta database path must be specified in nextflow.config, expect large disk space requirements for bakta light db"
    }
    ...
    if (params.graph != "specify_path") {
        ...
        error "Usage: nextflow run main.nf --graph <path_to_graph>"
    }
}